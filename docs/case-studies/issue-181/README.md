...
- **meta-language is a young Rust crate (v0.45.0, 0 stars, 694 downloads) built on
  the same `links-notation 0.13` RML already uses, with lossless representation,
  structural query/codemod, and many-valued/probabilistic truth values** — i.e. it
  is a strong fit for RML's needs. But it has **no npm package** (registry → 404),
  while RML must keep its JS and Rust implementations at parity. That single
  constraint drives the integration design.
...
projections, converging with [issue #138](../issue-138/) (MX4). The hard part is
giving the **JS** side access despite the missing npm package — solved behind a
`MetaLang` façade backed by wasm/upstream-publish, with a JS port as fallback.
...
4. **Parity behind a façade.** RML codes against a `MetaLang` interface; the JS
   backing (wasm vs port) is swappable, so the no-npm constraint never forks the
   design (N1).
...
- **JS-bridge** wasm/upstream/port to give JS access (riskiest task).
...
See [`risks-and-open-questions.md`](./risks-and-open-questions.md). The four for the
maintainer: (1) preferred JS bridge (upstream npm publish vs RML-owned wasm/port);
(2) whether Phase MX absorbs or runs beside [issue #138](../issue-138/); (3) the bar
to flip `--via-meta-language` to default; (4) whether to authorise auto-creating the
sub-issues.
...
- [`link-foundation/meta-language`](https://github.com/link-foundation/meta-language) — repo, README, crate v0.45.0 (crates.io), homepage <https://link-foundation.github.io/meta-language/>.
