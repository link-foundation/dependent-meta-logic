...
| **R3** | meta-language must be **extensible and feature-rich enough to support all our features**. | Partial (upstream) | meta-language now ships **both** a Rust crate (v0.45.0) and a native JavaScript package (`@link-foundation/meta-language` v0.46.0) with enforced Rust↔JS parity. Both offer lossless parse, projections, `LinkQuery`, `find/replace`, `SubstitutionRule`, `TranslationRuleSet`; many-valued/`ProbabilisticTruthValue` semantics remain Rust-only (JS parity gap [#166](https://github.com/link-foundation/meta-language/issues/166)) ([`data/online-research.md`](./data/online-research.md) §1, §5). A feature-coverage audit (does it cover *every* RML construct?) is itself a sub-issue (MX1). |
